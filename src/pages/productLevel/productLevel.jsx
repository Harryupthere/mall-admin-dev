...
import "./productLevel.scss";

const ProductLevelsForm = () => {
  ...
  const getDetailsById = async () => {
    try {
      const res = await fetchData(`${API_ENDPOINTS.getProductLevel}?id=${id}`, navigate, "GET");
      if (res?.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = isEditMode
      ? `${API_ENDPOINTS.updateProductLevel}/${id}`
      : API_ENDPOINTS.createProductLevel;
    ...
    try {
      ...
      if (res?.success) {
        ...
        navigate(`${process.env.REACT_APP_BASE_URL}product-levels`);
      } else {
  ...
  return (
    <div className="trade-form-container">
      <div className="trade-form-card">
        <h2>{isEditMode ? "Edit Product Level" : "Add Product Level"}</h2>
...
export default ProductLevelsForm;
