...
const CouponCodesForm = () => {
  ...
  const getCouponById = async () => {
    try {
      const res = await fetchData(`${API_ENDPOINTS.getCouponCodes}?id=${id}`, navigate, "GET");
      if (res?.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = isEditMode
      ? `${API_ENDPOINTS.updateCouponCodes}/${id}`
      : API_ENDPOINTS.createCouponCodes;
  ...
  return (
    <div className="trade-form-container">
      <div className="trade-form-card">
        <h2>{isEditMode ? "Edit Coupon" : "Add Coupon"}</h2>
